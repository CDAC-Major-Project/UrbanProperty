  ...
  return (
    <div className='w-full h-min-screen bg-[#F9FAFB] ' >
        ...
        <div className=' mt-35 w-screen h-[80vh] relative' >
            <img src={property?.photo} className='object-cover object-center aspect-video w-full h-full' />
            <div className='absolute bottom-10 left-1/2 -translate-x-1/2 p-4 bg-black/80 w-11/12 rounded-2xl flex justify-between ' >
                {/* property name and address */}
                <div>
                    <h1 className='text-4xl text-white font-bold'>{property?.title}</h1> 
                    <p className='text-lg  text-white flex items-center '> <PlaceOutlinedIcon fontSize="small" /> {property?.location}</p>
                </div>
                {/* property price */}
                <div className='flex flex-col items-end' >
                    <h2 className='text-3xl font-semibold text-white'>₹{property?.price.toLocaleString()}</h2>
                    <p className='font-semibold text-white'>₹{property?.area_sqft}/ sqft</p>
                </div>
            </div>
        </div>
            ...
            <div className='w-full grid grid-cols-3 space-x-5' >
                <div className='col-span-2 space-y-5 ' >
                    <div className=' h-fit w-full border-2 border-gray-300 bg-white items-center grid grid-cols-4 rounded-2xl shadow-gray-300 shadow-lg p-10' >
                        <div className='col-span-1 flex  space-x-5 ' >
                            ...
                            <div>
                                ...
                                <p className='text-[#BBBBBB]' >Bedrooms</p>
                            </div>
                        </div>
                        <div className='col-span-1 flex space-x-5 ' >
                    ...
                    <div className=' space-y-2 border-2 border-gray-300 bg-white rounded-2xl shadow-gray-300 shadow-lg p-5' >
                        ...
                        <p className='text-gray-600' >Experience luxury living in this stunning modern villa featuring breathtaking ocean views, premium finishes, and an open-concept design. This architectural masterpiece offers the perfect blend of comfort and sophistication, with floor-to-ceiling windows that flood the space with natural light and showcase panoramic views of the Pacific Ocean.</p>
                    </div>
                ...
                <div className=' h-fit border-2 border-gray-300 bg-white rounded-2xl shadow-gray-300 shadow-lg p-8 col-span-1 space-y-2 ' >
                    ...
                    <p className='text-center text-[#00A63E] font-bold text-3xl' >₹{property?.price.toLocaleString()}</p>
                ...
                <div className='flex flex-wrap gap-2' >
                    {
                        amenities?.length > 0 && amenities?.map((amenity, index) => (
                            <div key={index} className='bg-[#BBBBBB] rounded-lg w-fit px-2 py-1' >
                                <p className='text-black text-base' >{amenity}</p>
                            </div>
                        ))
                    }
                </div>
