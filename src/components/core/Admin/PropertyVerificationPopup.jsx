...
import { verifyPendingProperty } from "../../../Services/adminAPI";
import { useSelector } from "react-redux";

const PropertyVerificationPopup = ({ data, cancel, setPendingProperties }) => {
  console.log("data : ", data);

  const {token} = useSelector((state) => state.auth);

  const clickHandler = () => {
    verifyPendingProperty( token, data?.id, cancel, setPendingProperties);
  }
      ...
      <div className="bg-white rounded-lg p-6 w-2/5 shadow-lg space-y-5 ">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600">
              Review property details and make verification decision
            </p>
          </div>
        ...
        <div className="grid grid-cols-2">
          <div className="col-span-1">
            ...
            <p className="text-gray-600 text-sm ">{data?.propertyTypeName}</p>
          </div>
          <div className="col-span-1">
